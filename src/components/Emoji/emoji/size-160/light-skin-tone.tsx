import React, {forwardRef, ForwardedRef} from 'react'

export type Props = {}

const EmojiLightSkinTone = forwardRef((props: Props, ref: ForwardedRef<HTMLElement>) => {
  const { ...otherProps } = props

  return (
   <figure ref={ref} {...otherProps}>
      <img
        alt="Emoji-light-skin-tone"
        src="data:image/png;base64,&#x3D;&#x3D;"
      />
    </figure>
  )
})

EmojiLightSkinTone.displayName = 'EmojiLightSkinTone'
EmojiLightSkinTone.defaultProps = {}

export default EmojiLightSkinTone

import React, {forwardRef, ForwardedRef} from 'react'

export type Props = {}

const EmojiDivide = forwardRef((props: Props, ref: ForwardedRef<HTMLElement>) => {
  const { ...otherProps } = props

  return (
   <figure ref={ref} {...otherProps}>
      <img
        alt="Emoji-divide"
        src="data:image/png;base64,&#x3D;&#x3D;"
      />
    </figure>
  )
})

EmojiDivide.displayName = 'EmojiDivide'
EmojiDivide.defaultProps = {}

export default EmojiDivide

import React, {forwardRef, ForwardedRef} from 'react'

export type Props = {}

const EmojiWhiteMediumSquare = forwardRef((props: Props, ref: ForwardedRef<HTMLElement>) => {
  const { ...otherProps } = props

  return (
   <figure ref={ref} {...otherProps}>
      <img
        alt="Emoji-white-medium-square"
        src="data:image/png;base64,&#x3D;"
      />
    </figure>
  )
})

EmojiWhiteMediumSquare.displayName = 'EmojiWhiteMediumSquare'
EmojiWhiteMediumSquare.defaultProps = {}

export default EmojiWhiteMediumSquare

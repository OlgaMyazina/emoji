import React, {forwardRef, ForwardedRef} from 'react'

export type Props = {}

const EmojiLargePurpleSquare = forwardRef((props: Props, ref: ForwardedRef<HTMLElement>) => {
  const { ...otherProps } = props

  return (
   <figure ref={ref} {...otherProps}>
      <img
        alt="Emoji-large-purple-square"
        src="data:image/png;base64,&#x3D;&#x3D;"
      />
    </figure>
  )
})

EmojiLargePurpleSquare.displayName = 'EmojiLargePurpleSquare'
EmojiLargePurpleSquare.defaultProps = {}

export default EmojiLargePurpleSquare

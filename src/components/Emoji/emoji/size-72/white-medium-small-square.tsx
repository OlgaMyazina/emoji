import React, {forwardRef, ForwardedRef} from 'react'

export type Props = {}

const EmojiWhiteMediumSmallSquare = forwardRef((props: Props, ref: ForwardedRef<HTMLElement>) => {
  const { ...otherProps } = props

  return (
   <figure ref={ref} {...otherProps}>
      <img
        alt="Emoji-white-medium-small-square"
        src="data:image/png;base64,"
      />
    </figure>
  )
})

EmojiWhiteMediumSmallSquare.displayName = 'EmojiWhiteMediumSmallSquare'
EmojiWhiteMediumSmallSquare.defaultProps = {}

export default EmojiWhiteMediumSmallSquare

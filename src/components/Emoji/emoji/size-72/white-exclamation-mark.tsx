import React, { forwardRef, ForwardedRef } from 'react'

export type Props = {}

const EmojiWhiteExclamationMark = forwardRef(
  (props: Props, ref: ForwardedRef<HTMLElement>) => {
    const { ...otherProps } = props

    return (
      <figure ref={ref} {...otherProps}>
        <img
          alt="Emoji-white-exclamation-mark"
          src="data:image/png;base64,&#x3D;"
        />
      </figure>
    )
  }
)

EmojiWhiteExclamationMark.displayName = 'EmojiWhiteExclamationMark'
EmojiWhiteExclamationMark.defaultProps = {}

export default EmojiWhiteExclamationMark
